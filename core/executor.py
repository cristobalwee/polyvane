...
class Executor:
    ...
    async def _observe_book_depth(self, token_id: str | None) -> dict[str, Any] | None:
        if not token_id or not self.client.is_initialized:
            return None
        try:
    ...
    async def submit(self, intent: TradeIntent, *, bankroll_usd: float) -> ExecutionResult:
        ...
        if sig.price is None:
            log.warning(
                "TRADE_REJECTED | market=%s | reason=missing_price",
                sig.market_id,
            )
            return ExecutionResult(False, reason="signal missing price")

        log.trade(
            "TRADE_SIGNAL | market=%s | bucket=%s | edge=%.4f | confidence=%.2f | price=$%.4f",
            sig.market_id, bucket, sig.edge, sig.confidence, sig.price,
        )

        book_depth = await self._observe_book_depth(sig.token_id)
        ...
        if not decision.approved:
            log.trade(
                "TRADE_REJECTED | market=%s | reason=%s",
                sig.market_id, decision.reason,
            )
        ...
        if self.config.is_paper:
            log.trade(
                "TRADE_PAPER | market=%s | bucket=%s | side=%s | price=$%.4f | size=$%.2f | shares=%.2f | edge=%.4f | trade_id=%d",
                sig.market_id, bucket, side_str, sig.price, size_usd, shares, sig.edge, trade_id,
            )
        ...
        try:
            ...
            log.exception(
                "TRADE_REJECTED | market=%s | reason=order_error | error=%r",
                sig.market_id, e,
            )
            return ExecutionResult(
                accepted=False,
        ...
        for resp in responses:
            ...
            log.trade(
                "TRADE_LIVE | market=%s | bucket=%s | side=%s | price=$%.4f | size=$%.2f | order_id=%s",
                sig.market_id, bucket, side_str, sig.price, size_usd, order_id,
            )
